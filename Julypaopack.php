<?php 
  ...
  class Paopack extends Configuration{
		  ...
		  function GetCategories(){
           ...
           /*Function to Get all Cities- 3*/
		  function GetCities(){
		  	$db=$this->CheckConnection();
			if($db){
				//echo "Connected";
				$json_city=array();
				$QueryToGetCities = $db->query("select * from tbl_pao_delivery_city");
				if($QueryToGetCities->num_rows>0){
					while($City = $QueryToGetCities->fetch_array()){
						$json_city[]=array("CityId"=>$City['id'],"CityTitle"=>$City ['city_name']);
					}
					return json_encode($json_city);
				}else{}
			}else{
				//echo "Not Connected";
				return json_encode($json_city);
        ...
        /*Function to Get Client Ip Address - 14*/
			...
			function PaopackAddtoCart($ProductId,$Email){
				$db=$this->CheckConnection();
				if($db){
					...
					if($SelectCartProducts->num_rows>0){
						...
						if($db->query("insert into tbl_pao_cart_products values('','2','$ProductId','".$this->PaopackClientIp()."','$Email',NOW(),'')")){
							return TRUE;
						}else{
							return FALSE;
						}
			...
			function PaopackCountCartProducts(){
				...
				if($db){
					...
					if($SelectProducts->num_rows>0){
						...
						$CountProducts = count(explode(",", $ProductDetails[2]));
            ...
            function IsPaopackAdminExists($Email,$Password,$UserType){
            	...
            	if($db){
            		if($UserType=="admin"){
            			...
            			$SelectAdmin = $db->query("select * from tbl_executives_registration where exe_email = '$Email' and exe_password = '$Password'");
            		}
            		$AdminDetails = array();
				...
				switch($OperationMode){
					case "Add" :
								...
								if($this->IsValidImage($Image)){
									$ImgName = $Image['name'];
									if($db->query("insert into tbl_pao_products values('','$CategoryId','$SubcatId','$Type','$Name','$Condition','$Desc','$Weight','$Price','$Discount','http://paopack.com/images/products/$ImgName',NOW())")){
									  return TRUE;
									}else{
										return FALSE;
									}
         ...
         function PaopackAddCity()
{
$db=$this->CheckConnection();
			if($db){
$Title = $_POST['city_title'];
						      if($db->query("insert into tbl_pao_delivery_city values('','$Title',NOW())")){
						      	return TRUE;
						      }else{
						      	return FALSE;
						      }
...
function PaopackAddLocality()
{
$db=$this->CheckConnection();
			if($db){
$Title = $_POST['locality_title'];
$City=$_POST['city'];
						      if($db->query("insert into tbl_pao_delivery_locality values('','$City','$Title',NOW())")){
						      	return TRUE;
						      }else{
						      	return FALSE;
						      }
...
case 'Update':
					...
					if($SelectProducts->num_rows>0){
						...
						foreach($ProductIds as $ProdId=>$Count){
							...
							foreach($ProductDetailsFromQuickView as $QuickView){
								$CartDetailsJson[]=array("Count"=>$Count,"ProductName"=>$QuickView->ProductName,"ProductImage"=>$QuickView->ProductImage,"Price"=>$QuickView->ProductDiscount,"ProductId"=>$ProductId,"Original"=>$QuickView->ProductPrice,"CartId"=>$ProductDetails[0],"DateTime"=>$ProductDetails[5]);
							}
						}
		 ...
		 function PaopackDeleteCartProduct($ProductId){
		 	$db=$this->CheckConnection();
				if($db){
					...
					if($SelectProducts->num_rows>0){
						$ProductDetails = $SelectProducts->fetch_array();
						$ProductIds = explode(",",$ProductDetails[2]);
		...
		function PaopackSaveDeliveryLocation($LoggedUserId){
			$db=$this->CheckConnection();
			if($db){
				if($db->query("insert into tbl_pao_saved_delivery_address values('','$LoggedUserId','".$_POST['address']."','".$_POST['city']."','".$_POST['pincode']."','".$_POST['country']."','".$_POST['mobile']."',NOW(),'".$_POST['lN']."')")){
						return TRUE;
				}else{
						return FALSE;
				}
				   ...
				   return FALSE;
					...
					if($image_height=="255" && $image_width=="770"){
						if(move_uploaded_file($_FILES["slider_image"]["tmp_name"], "../images/slider/".$_FILES["slider_image"]["name"])){
							if($db->query("insert into tbl_paopack_slider_images values('','paopack.com/images/slider/".$_FILES["slider_image"]["name"]."','active',NOW())")){
								return TRUE;
							}else{
								return FALSE;
							}
