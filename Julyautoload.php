<?php 
	  ...
	  $Cart = isset($_GET['tocart'])?$_GET['tocart']:"";$ProductId = isset($_GET['pid'])?$_GET['pid']:"";$ProductView = isset($_GET['view'])?$_GET['view']:"";$Email = isset($_GET['Email '])?$_GET['Email ']:"";
	  if($Cart=="add"){
	  	    $PIds = $paopack->GetPageView($ProductId);
			if($paopack->PaopackAddtoCart($PIds,$Email)){
				if(empty($ProductView)){
					header("location:./");
				}else{
					header("location:paopack_view.php?view=".$ProductView."&pid=".$ProductId."&Email=".$Email);
				}
			}else{
	  ...
	  if(isset($_POST['btnAddCity'])){
	  	if($paopack->PaopackAddCity()){
	  		$Style = "color:Green;font-weight:bold;";
	  		$MessageStatus = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Added Successfully.";
	  	}
	  }
     /*Function to Add Locality*/
	  if(isset($_POST['btnAddLocality'])){
	  	if($paopack->PaopackAddLocality()){
	  		$Style = "color:Green;font-weight:bold;";
	  		$MessageStatus = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Added Successfully.";
	  	}
	  }
